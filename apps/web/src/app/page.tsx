...
export default function LandingPage() {
  ...
  const canSubmit = file !== null && !submitting && jd.length >= JD_MIN_LENGTH;
  ...
  return (
    <>
      ...
      <section className="border-border/50 relative overflow-hidden border-b">
        ...
        <div
          className="via-background/80 to-background absolute inset-0 bg-gradient-to-b from-transparent"
          aria-hidden
        />
        <div className="container relative grid gap-10 py-16 md:grid-cols-[1.1fr_1fr] md:gap-16 md:py-24">
          <div className="flex flex-col justify-center">
            <div className="border-border/60 bg-card/50 text-muted-foreground mb-6 inline-flex w-fit items-center gap-2 rounded-full border px-3 py-1 text-xs">
              <span className="animate-subtle-pulse bg-primary inline-block h-1.5 w-1.5 rounded-full" />
          ...
          <form
            ...
            className="border-border bg-card/60 flex flex-col gap-4 rounded-xl border p-5 shadow-sm md:p-6"
          >
      ...
      <section className="border-border/50 border-b py-16 md:py-20">
      ...
      <section className="border-border/50 border-b py-16 md:py-20">
        <div className="container max-w-3xl">
          ...
          <p className="text-muted-foreground mt-3">
            Most AI resume tools will happily invent a 40% improvement metric or add &ldquo;led a
            team of 8&rdquo; when your bullet said &ldquo;worked with the team.&rdquo; That&apos;s
            how people get caught in interviews. Here are the guardrails.
          </p>
...
function Step({
  ...
  return (
    <div className="border-border/60 bg-card/40 hover:border-primary/30 hover:bg-card/70 group relative rounded-xl border p-5 transition-colors">
      <div className="mb-4 flex items-center justify-between">
        <span className="text-muted-foreground font-mono text-xs">{index}</span>
        <div className="border-border/60 bg-background text-muted-foreground group-hover:text-primary flex h-7 w-7 items-center justify-center rounded-md border">
          {icon}
        </div>
      </div>
